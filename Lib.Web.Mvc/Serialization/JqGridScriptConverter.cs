...
namespace Lib.Web.Mvc.JqGridFork.Serialization
{
	internal class JqGridScriptConverter : JavaScriptConverter
	{
		...
		private static readonly ReadOnlyCollection<Type> _supportedTypes = new ReadOnlyCollection<Type>(new List<Type>()
		{
			...
			typeof(JqGridTreeIcons),
			typeof(JqGridNestedTreeReader),
			typeof(JqGridAdjacencyTreeReader)
		});
		#endregion
		...
		public override object Deserialize(IDictionary<string, object> dictionary, Type type, JavaScriptSerializer serializer)
		{
			...
			if (dictionary != null)
			{
				...
				else if (type == typeof(JqGridTreeIcons))
					obj = DeserializeJqGridTreeIcons(dictionary, serializer);
				else if (type == typeof(JqGridNestedTreeReader))
		...
		public override IDictionary<string, object> Serialize(object obj, JavaScriptSerializer serializer)
		{
			...
			if (obj != null)
			{
				...
				else if (obj is JqGridTreeIcons)
					SerializeJqGridTreeIcons((JqGridTreeIcons)obj, serializer, serializedObj);
				else if (obj is JqGridNestedTreeReader)
		...
		private static JqGridOptions DeserializeJqGridOptions(IDictionary<string, object> serializedObj, JavaScriptSerializer serializer)
		{
			...
			if (!string.IsNullOrWhiteSpace(id))
			{
				...
				obj.EditNextRowCell = GetBooleanFromSerializedObj(serializedObj, "editNextRowCell");
				...
				obj.PagerButtons = GetBooleanFromSerializedObj(serializedObj, "pgbuttons");
				obj.PagerInput = GetBooleanFromSerializedObj(serializedObj, "pginput");

				if (serializedObj.ContainsKey("pgtext"))
					obj.PagerText = serializedObj["pgtext"]?.ToString();
				...
				obj.PreserveSelection = GetBooleanFromSerializedObj(serializedObj, "preserveSelection");
				...
				obj.TreeGridBigData = GetBooleanFromSerializedObj(serializedObj, "treeGrid_bigData");
				obj.TreeRootLevel = GetInt32FromSerializedObj(serializedObj, "tree_root_level");
				if (serializedObj.ContainsKey("treeReader") && serializedObj["treeReader"] is IDictionary<string, object> reader)
				{
					if (reader.ContainsKey("left_field") || reader.ContainsKey("right_field"))
						obj.TreeReader = DeserializeJqGridNestedTreeReader(reader, serializer);
					else
						obj.TreeReader = DeserializeJqGridAdjacencyTreeReader(reader, serializer);
				}

				if (serializedObj.ContainsKey("treeIcons") && serializedObj["treeIcons"] is IDictionary<string, object> icons)
					obj.TreeIcons = DeserializeJqGridTreeIcons(icons, serializer);
		...
		private static void SerializeJqGridOptions(JqGridOptions obj, JavaScriptSerializer serializer, Dictionary<string, object> serializedObj)
		{
			...
			if (obj.DynamicScrollingTimeout.HasValue)
				serializedObj.Add("scrollTimeout", obj.DynamicScrollingTimeout.Value);

			if (obj.EditNextRowCell.HasValue)
				serializedObj.Add("editNextRowCell", obj.EditNextRowCell.Value);
			...
			if (obj.Pager)
			{
				...
				if (obj.PagerButtons.HasValue)
					serializedObj.Add("pgbuttons", obj.PagerButtons.Value);

				if (obj.PagerInput.HasValue)
					serializedObj.Add("pginput", obj.PagerInput.Value);

				if (obj.IsPagerTextSetted)
					serializedObj.Add("pgtext", obj.PagerText);
			}
			...
			if (obj.PreserveSelection.HasValue)
				serializedObj.Add("preserveSelection", obj.PreserveSelection.Value);
			...
			if (obj.TreeGridEnabled.HasValue)
			{
				...
				if (obj.TreeGridEnabled.Value)
				{
					...
					if (obj.TreeRootLevel.HasValue)
						serializedObj.Add("tree_root_level", obj.TreeRootLevel.Value);

					if (obj.TreeGridBigData.HasValue)
						serializedObj.Add("treeGrid_bigData", obj.TreeGridBigData.Value);

					if (obj.TreeReader != null)
						serializedObj.Add("treeReader", obj.TreeReader);

					if (obj.TreeIcons != null)
						serializedObj.Add("treeIcons", obj.TreeIcons);
				}
			}
		...
		private static JqGridColumnModel DeserializeJqGridColumnModel(IDictionary<string, object> serializedObj, JavaScriptSerializer serializer)
		{
			...
			if (!string.IsNullOrWhiteSpace(name))
			{
				...
				obj.Tooltip = GetSettedStringFromSerializedObj(serializedObj, "tooltip");
				obj.Width = GetInt32FromSerializedObj(serializedObj, "width");
		...
		private static void SerializeJqGridColumnModel(JqGridColumnModel obj, JavaScriptSerializer serializer, Dictionary<string, object> serializedObj)
		{
			...
			if (obj.Tooltip?.IsSetted ?? false)
				serializedObj.Add("tooltip", obj.Tooltip.Value);
		...
		private static JqGridTreeReader DeserializeJqGridTreeReader(JqGridTreeReader obj, IDictionary<string, object> serializedObj, JavaScriptSerializer serializer)
		{
			if (obj == null) throw new ArgumentNullException(nameof(obj));

			obj.LevelField = GetStringFromSerializedObj(serializedObj, "level_field");
			obj.LeafField = GetStringFromSerializedObj(serializedObj, "leaf_field");
			obj.ExpandedField = GetStringFromSerializedObj(serializedObj, "expanded_field");
			obj.Loaded = GetStringFromSerializedObj(serializedObj, "loaded");
			obj.IconField = GetStringFromSerializedObj(serializedObj, "icon_field");

			return obj;
		}

		private static bool IsNotEmptyTreeReader(JqGridTreeReader obj)
		{
			return !string.IsNullOrWhiteSpace(obj.LevelField) ||
				   !string.IsNullOrWhiteSpace(obj.LeafField) ||
				   !string.IsNullOrWhiteSpace(obj.ExpandedField) ||
				   !string.IsNullOrWhiteSpace(obj.Loaded) ||
				   !string.IsNullOrWhiteSpace(obj.IconField);
		}

		private static void SerializeJqGridTreeReader(JqGridTreeReader obj, JavaScriptSerializer serializer, Dictionary<string, object> serializedObj)
		{
			if (!string.IsNullOrWhiteSpace(obj.LevelField))
				serializedObj.Add("level_field", obj.LevelField);

			if (!string.IsNullOrWhiteSpace(obj.LeafField))
				serializedObj.Add("leaf_field", obj.LeafField);

			if (!string.IsNullOrWhiteSpace(obj.ExpandedField))
				serializedObj.Add("expanded_field", obj.ExpandedField);

			if (!string.IsNullOrWhiteSpace(obj.Loaded))
				serializedObj.Add("loaded", obj.Loaded);

			if (!string.IsNullOrWhiteSpace(obj.IconField))
				serializedObj.Add("icon_field", obj.IconField);
		}

		private static JqGridNestedTreeReader DeserializeJqGridNestedTreeReader(IDictionary<string, object> serializedObj, JavaScriptSerializer serializer)
		{
			JqGridNestedTreeReader obj = new JqGridNestedTreeReader();
			obj = (JqGridNestedTreeReader)DeserializeJqGridTreeReader(obj, serializedObj, serializer);

			obj.LeftField = GetStringFromSerializedObj(serializedObj, "left_field");
			obj.RightField = GetStringFromSerializedObj(serializedObj, "right_field");

			if (IsNotEmptyTreeReader(obj) || !string.IsNullOrWhiteSpace(obj.LeftField) ||
				!string.IsNullOrWhiteSpace(obj.RightField))
				return obj;
			return null;
		}

		private static void SerializeJqGridNestedTreeReader(JqGridNestedTreeReader obj, JavaScriptSerializer serializer, Dictionary<string, object> serializedObj)
		{
			SerializeJqGridTreeReader(obj, serializer, serializedObj);

			if (!string.IsNullOrWhiteSpace(obj.LeftField))
				serializedObj.Add("left_field", obj.LeftField);

			if (!string.IsNullOrWhiteSpace(obj.RightField))
				serializedObj.Add("right_field", obj.RightField);
		}

		private static JqGridAdjacencyTreeReader DeserializeJqGridAdjacencyTreeReader(IDictionary<string, object> serializedObj, JavaScriptSerializer serializer)
		{
			JqGridAdjacencyTreeReader obj = new JqGridAdjacencyTreeReader();
			obj = (JqGridAdjacencyTreeReader)DeserializeJqGridTreeReader(obj, serializedObj, serializer);

			obj.ParentIdField = GetStringFromSerializedObj(serializedObj, "parent_id_field");

			if (IsNotEmptyTreeReader(obj) || !string.IsNullOrWhiteSpace(obj.ParentIdField))
				return obj;
			return null;
		}

		private static void SerializeJqGridAdjacencyTreeReader(JqGridAdjacencyTreeReader obj, JavaScriptSerializer serializer, Dictionary<string, object> serializedObj)
		{
			SerializeJqGridTreeReader(obj, serializer, serializedObj);

			if (!string.IsNullOrWhiteSpace(obj.ParentIdField))
				serializedObj.Add("parent_id_field", obj.ParentIdField);
		}

		private static JqGridTreeIcons DeserializeJqGridTreeIcons(IDictionary<string, object> serializedObj, JavaScriptSerializer serializer)
		{
			JqGridTreeIcons obj = new JqGridTreeIcons();

			obj.PlusIcon = GetStringFromSerializedObj(serializedObj, "plus");
			obj.MinusIcon = GetStringFromSerializedObj(serializedObj, "minus");
			obj.LeafIcon = GetStringFromSerializedObj(serializedObj, "leaf");

			if (!string.IsNullOrWhiteSpace(obj.PlusIcon) || !string.IsNullOrWhiteSpace(obj.MinusIcon) || !string.IsNullOrWhiteSpace(obj.LeafIcon))
				return obj;
			return null;
		}

		private static void SerializeJqGridTreeIcons(JqGridTreeIcons obj, JavaScriptSerializer serializer, Dictionary<string, object> serializedObj)
		{
			if (!string.IsNullOrWhiteSpace(obj.PlusIcon))
				serializedObj.Add("plus", obj.PlusIcon);

			if (!string.IsNullOrWhiteSpace(obj.MinusIcon))
				serializedObj.Add("minus", obj.MinusIcon);

			if (!string.IsNullOrWhiteSpace(obj.LeafIcon))
				serializedObj.Add("leaf", obj.LeafIcon);
		}
