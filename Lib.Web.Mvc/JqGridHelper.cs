...
namespace Lib.Web.Mvc.JqGridFork
{
    ...
    public class JqGridHelper<TModel> : IJqGridHelper
    {
        ...
        public MvcHtmlString GetJavaScript()
        {
            ...
            if (_navigatorOptions != null || _inlineNavigatorOptions != null)
        ...
        private void AppendOptions(StringBuilder javaScriptBuilder)
        {
            ...
            if (_options.Pager)
            {
                ...
                if (_options.PagerButtons.HasValue)
                    javaScriptBuilder.AppendFormat("pgbuttons: {0},", _options.PagerButtons.Value.ToString().ToLower()).AppendLine();

                if (_options.PagerInput.HasValue)
                    javaScriptBuilder.AppendFormat("pginput: {0},", _options.PagerInput.Value.ToString().ToLower()).AppendLine();
            ...
            if (_options.TreeGridEnabled.HasValue)
            {
                ...
                if (_options.TreeGridEnabled.Value)
                {
                    ...
                    if (_options.TreeRootLevel.HasValue)
                        javaScriptBuilder.AppendFormat("tree_root_level: {0},", _options.TreeRootLevel.Value).AppendLine();

                    if (_options.TreeGridBigData.HasValue)
                        javaScriptBuilder.AppendFormat("treeGrid_bigData: {0},", _options.TreeGridBigData.Value.ToString().ToLower()).AppendLine();
        ...
        {
            if (_options.TreeIcons == null) return;
            javaScriptBuilder.Append("treeIcons: { ");

            if (!string.IsNullOrWhiteSpace(_options.TreeIcons.PlusIcon))
                javaScriptBuilder.AppendFormat("plus: '{0}', ", _options.TreeIcons.PlusIcon);

            if (!string.IsNullOrWhiteSpace(_options.TreeIcons.MinusIcon))
                javaScriptBuilder.AppendFormat("minus: '{0}', ", _options.TreeIcons.MinusIcon);

            if (!string.IsNullOrWhiteSpace(_options.TreeIcons.LeafIcon))
                javaScriptBuilder.AppendFormat("leaf: '{0}', ", _options.TreeIcons.LeafIcon);
        ...
        {
            if (_options.TreeReader == null) return;
            javaScriptBuilder.Append("treeReader: { ");

            if (!string.IsNullOrWhiteSpace(_options.TreeReader.LevelField))
                javaScriptBuilder.AppendFormat("level_field: '{0}', ", _options.TreeReader.LevelField);

            if (!string.IsNullOrWhiteSpace(_options.TreeReader.LeafField))
                javaScriptBuilder.AppendFormat("leaf_field: '{0}', ", _options.TreeReader.LeafField);

            if (!string.IsNullOrWhiteSpace(_options.TreeReader.ExpandedField))
                javaScriptBuilder.AppendFormat("expanded_field: '{0}', ", _options.TreeReader.ExpandedField);

            if (!string.IsNullOrWhiteSpace(_options.TreeReader.Loaded))
                javaScriptBuilder.AppendFormat("loaded: '{0}', ", _options.TreeReader.Loaded);

            if (!string.IsNullOrWhiteSpace(_options.TreeReader.IconField))
                javaScriptBuilder.AppendFormat("icon_field: '{0}', ", _options.TreeReader.IconField);

            if (_options.TreeReader is JqGridNestedTreeReader nestedTreeReader)
            {
            ...
            if (_options.TreeReader is JqGridAdjacencyTreeReader adjacencyTreeReader)
            {
